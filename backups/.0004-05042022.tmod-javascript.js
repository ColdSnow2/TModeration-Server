globalThis.tmoderation = () => {
  ...
  try {
    ...
    botRunner.on('messageCreate', async msg => { // Byebye invites
      if (msg.content.includes('discord.gg') || msg.content.includes('https://discord.gg') || msg.content.includes('https://discord.com/invite') || msg.content.includes('https://bit.ly')) {
        if (msg.author != "937190474299633734") {
          msg.delete()
          msg.channel.send("In mind! From TModeration Bot v25.2.1, we have integrated Invitation Link Prevention! Feels free keep sending links, but sending invites is BANNED")
        } else {
          return false
        }
      }
    ...
    botRunner.on('messageCreate', async (msg) => { //DiscordJS v13 no longer supports message, messageCreate instead
      ...
      if(msg.member.displayName.startsWith("!")) {
        msg.guild.fetchOwner(o => {
          msg.channel.send("Detected new hoisting member. <@" + o.user.id + ">")
        })
      }
      ...
      if(msg.content.startsWith('tm ')) {
        if(msg.author.id != "927563409409581066") {
          ...
          msg.channel.send("TMod. is now passive")
          msg.channel.send("TMod. is under attack, and we want safe for everyone. We will see again in 14 hours next. Thanks")
          return false
        }
      }
      ...
      switch (command) {
        ...
        case 'eval':
          ...
          let tmod__FFilLeSSYsTeeM__55021_11911_D02M05Y2022_ = require("fs");
          let tmod__FilESy_2202_299438HF = {};
          let tmod__botRunneR_DESTROY_28239_49211_9221K_0201 = botRunner.destroy
  
          delete botRunner.destroy
          try {
            ...
            // Bye processes
            delete process.kill
            delete process.env.REPL_SYSTEM_ID
            
            if (msg.content.match(/botRunner *. *user *. *(setAvatar|setUsername)/gim)) {
              msg.channel.send("```js\nfalse```");
              return false
            }
            ...
            let out = eval(code.join(' ')) // Eval the splited code
            ...
            out = out.replace(fs.toString(), 'null')
            msg.channel.send("```js\n" + out + '```')
          } catch (e) {
            msg.channel.send("```js\n" + e.toString() + "\n```")
          }
        ...
        case "beg":
          ...
          const messagesList = [
            'ew no',
            'I hate chu',
            'coins.dll and CoinsGiveawayByBegging.exe has stopped working'
            , 'lmao no'
            , 'ur a jerk'
          ]
        ...
        case 'avatar':
          msg.channel.send(msg.mentions.members.first().displayAvatarURL)
          break
        
        case 'restart':
          msg.channel.send("Restarting TModeration Bot...\nCryingCore is restarting...")
          require('./restarter.js')
          globalThis.restart()
          break
      }
